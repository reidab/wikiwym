/**
    ...
    - The opener for UL/OL elements are sometimes (haven't yet localized where)
    getting lost (not output). Since the logic is the same as for TABLE, i presume
    that tables and maybe {{{...}}} blocks might also be affected.
...
GoogleCodeWikiParser.prototype.parse = function(text)
{
    ...
    for( i = 0; i < end; ++i )
    {
        ...
        for( k in this.rx.block )
        {
            ...
            x = i;
            var rc;
            for( ; (i<end) && (rc = check.doLine(ln,['discarding_this'])) > 0; )
            {
                isblock[i] = true;
    ...
    for( i = 0; i < this.lines.length; ++i )
    {
        ...
        for( k in this.rx.block ) {
            ...
            if( bl.rx.test( ln ) )
            {
                ...
                var res = [];
                for( ;  (i<end) && (rc = bl.doLine(ln,res)) > 0; )
                {
                    ln = this.lines[++i];
                    //print("TRYING",k,"ON LINE:",ln);
                }
