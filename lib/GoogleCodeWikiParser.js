...
function GoogleCodeWikiParser() {
  ...
  this.rx = {
    ...
    headers: [
      ...
      [/^#sidebar.*$/i, function(){ return ps.getWarning("The `#sidebar` directive is not supported by this parser!")}]
    ],
    ...
    block: {
      ...
      listOL: {
        ...
        * The only difference is one line of code, but i need two handlers in order to tell
        * if two list types are immediately consecutive in the input.
        * 
        * FIXME: doesn't support nested lists, but the rx.match()[0] field is there to
        ...
        */
        doLine: function(line,result) {
          ...
          result.push('<li>', ps.parseInlined(m[3]), '</li>');
          return 1;
        }
      }/*listOL*/,
...
GoogleCodeWikiParser.prototype.createLink = function(where,label) {
  ...
  return '<a href="'+where+'">'+(label || where)+'</a>';
}
