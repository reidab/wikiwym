/**
    ...
    - It is regex-based and therefore fragile.

    - Inline markup is only matched across a single line.
    ...
    this.rx =
    {
        ...
        */
        ...
        [
            ...
            // Wiki words
            [/!([A-Z][a-z0-9_]+)/g, '$1'],
            [/\[([A-Z][a-zA-Z0-9_]+)\]/g, '<a href="$1">$1</a>'],
            [/\[([A-Z][a-zA-Z0-9_]+) +([^\]]*)\]/g, '<a href="$1">$2</a>'],
            //[/\b([A-Z][a-zA-Z0-9_]+)/g, '<a href="$1">$1</a>'], // can't do this easily with a regex-based parser. It hoses CamelCast text in links.
...
GoogleCodeWikiParser.prototype.parse = function(text)
{
    ...
    {
        ln = this.lines[i];
        // This BR handling isn't quite right: breaks between paragraphs are missing. If i add another BR then i get too many, though.
        if( i && !ln.length )
        {
            ++emptyCount;
            continue;
